...
const BookPage = forwardRef<HTMLDivElement, { panel: ReaderPanel; onExpand: () => void }>(
...
export function ComicReader({
  ...
  const aspectRatio =
    aspect === "landscape" ? 16 / 9 : aspect === "square" ? 1 : 3 / 4;
  ...
  return (
    <>
      ...
      <div className="fixed inset-x-0 bottom-[calc(var(--mobile-nav-h)+var(--safe-bottom))] z-30 flex items-center justify-between gap-2 border-t border-subtle bg-background/90 px-4 py-3 backdrop-blur sm:hidden">
        <button
          type="button"
          onClick={flipPrev}
          disabled={page === 0}
...
function PageLightbox({
  ...
  return (
    ...
    >
      <div
        className="flex items-center justify-between px-4 py-3 text-[12px] text-white/75"
